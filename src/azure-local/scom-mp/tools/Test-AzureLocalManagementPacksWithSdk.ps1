...
<#
.SYNOPSIS
    Verifies generated Azure Local Management Packs with Microsoft VSAE and the Operations Manager SDK.
...
.PARAMETER InputPath
    Directory containing generated Azure Local Management Pack XML and sealed product MP files.
...
$artifactOrder = @(
    'HybridSolutionsCloud.AzureLocal.Library.xml',
    'HybridSolutionsCloud.AzureLocal.Discovery.xml',
    'HybridSolutionsCloud.AzureLocal.Monitoring.xml',
    'HybridSolutionsCloud.AzureLocal.Presentation.xml',
    'HybridSolutionsCloud.AzureLocal.Reporting.xml'
)
...
if ($verified.Count -eq 0) {
    throw "No Azure Local Management Pack XML files were found in '$resolvedInput'."
}

Write-Output "Microsoft VSAE/SDK verification passed for $($verified.Count) Azure Local Management Pack artifact(s)."
